...
- If `reasoning_logs/` contains new markdown logs, `reasond` prompts to process them.
...
- `.reasond/`
- `reasoning_logs/`
- `.gitignore` entries for `.reasond/` and `reasoning_logs/`

Install is merge-safe and idempotent:

- Managed blocks in `AGENTS.md` and `CLAUDE.md` are upserted between `REASONING-LOGS` markers.
...
- `.reasond/judge_output_schema.json`
...
- `audit_sources`
  One row per indexed markdown file under `reasoning_logs/`.
- `audit_runs_codex`
